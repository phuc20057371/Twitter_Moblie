...
import { formatDateTime } from '../helper/formatdate.helper';
...
export const getTweetByUserName: RequestHandler = async (req, res) => {
  try {
    ...
    const tweetsWithFormattedDate = tweetFind.map((tweet) => ({
      ...tweet.toObject(),
      dateTweet: formatDateTime(tweet.dateTweet.toISOString()),
    }));
    res.status(200).json(tweetsWithFormattedDate);
  } catch (error) {
...
//     console.log('d', tweetId);
//     // const tweetFind = await Tweet.find({ _id: tweetId }).sort({ dateTweet: -1 });
//     // const tweetsWithFormattedDate = tweetFind.map((tweet) => ({
//     //   ...tweet.toObject(),
//     //   dateTweet: formatDateTime(tweet.dateTweet.toISOString()),
//     // }));
//     // console.log('dd', tweetFind);
//     // res.status(200).json(tweetsWithFormattedDate);
//     const tweetFind = await Tweet.findById({ _id: tweetId });
//     console.log('twett: ', tweetFind);
//     if (!tweetFind) throw new Error('User not found');
...
export const updateCountLike: RequestHandler = async (req, res) => {
  try {
    ...
    if (tweet.likes.find((element) => element.userName === user.userName)) {
      const index = tweet.likes.indexOf({ userName: user.userName });
      tweet.likes.splice(index, 1);
    } else {
      tweet.likes.push({ userName: user.userName });
    }
